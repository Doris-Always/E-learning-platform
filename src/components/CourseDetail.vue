<template>
  <p class="text-color mx-8 py-4">
    My Courses / {{ course.category }} /
    <span class="text-black"> {{ course.name }}</span>
  </p>
  ...
  <div v-if="course && course.instructor" class="mx-8">
    ...
    <div
      ...
      :style="{ backgroundImage: `url(${course.coursePicture})` }"
    >
    ...
    <v-tabs class="mb-8 mx-0" v-model="activeTab" grow >
      <v-tab key="Description">Description</v-tab>
      <v-tab key="Reviews">Reviews</v-tab>
      <v-tab key="Discussion">Discussion</v-tab>
      <v-tab key="Resources">Resources</v-tab>
    </v-tabs>
    <hr class="border mb-8 mx-0" />
    <v-row>
      <v-tabs-items v-model="activeTab">
        ...
        <v-tab-item key="Resources">
          <ul>
            <li v-for="(resource, index) in course.resources" :key="index">
              {{ resource }}
            </li>
          </ul>
        </v-tab-item>
